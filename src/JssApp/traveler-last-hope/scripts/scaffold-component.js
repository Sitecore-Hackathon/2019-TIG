...
console.log(`* Implement the React component in ${chalk.green(componentOutputPath.outputReactFilePath)}`);
console.log(`* Add the styles of component in ${chalk.green(componentOutputPath.outputStylesFilePath)}`);
if (manifestOutputPath) {
  console.log(
...
const ${exportVarName} = (props) => (
  <PlaceholderComponent {...props}>
    <div className="c-${componentName}">
      <RichText field={props.fields.heading} tag='p' />
      <br />
      <p>${componentName} Component</p>
      <br />
      <p>{getFieldValue(props.fields, 'heading')}</p>
    </div>
...
`;
  ...
  const outputReactFilePath = path.join(outputDirectoryPath, 'index.js');
  fs.writeFileSync(outputReactFilePath, componentTemplate, 'utf8');

  const outputStylesFilePath = path.join(outputDirectoryPath, 'styles.scss');
  fs.writeFileSync(outputStylesFilePath, componentStyles, 'utf8');

  return {
    outputReactFilePath,
    outputStylesFilePath
  };
}
